...
class gMAE(nn.Module):
    ...
    def forward(self, value, truth_time_steps, mask):
        ...
        here now mask (B,L,2*d)
        now the L has changed to the max_len for all sample in the dataset, so each batch
        the training shape is the same for now! 
        """
        B, L, D = value.shape
        ...
        #check how many 1s are in the interp_mask
        # check wheter the interp_mask is identical to the original mask
        if (interp_mask-mask).sum() != 0:
            print('interp_mask and original mask are not identical!')
        else:
            print('interp_mask and original mask are identical!')
        sys.exit()
        ...
        X_value = X.clone()  # Keep a copy of the original value for reconstruction
        ...
        # Only include the value itself, not time embeddings (those go in key/query)
          # Just the actual values (B*D, L, 1)
        ...
        ###!!! the projector is recreated with random weights in every forward pass!
        # such projects weights are randomly initialized in every forward pass
        # there;s no learning for signal reconstruction in the pretrain...so the network can learn
        # good latent representatons, but cannot learn to project these back to the original space
        ...
        # ## only compute the loss on the masked patches, calculate the mse for the interp mask
        ...
        # # Combined loss
        alpha = 0.7  # Weight or signal vs latent loss
