...
class gMAE(nn.Module):
    def __init__(self, config):
        ...
        self.patch_dim = self.hid_dim // self.patch_size
        ...
        decoder_layer = nn.TransformerDecoderLayer(
            d_model=self.hid_dim // self.patch_size,
            nhead=4,  # Fewer heads than encoder
            dim_feedforward=2 * (self.hid_dim // self.patch_size),  # Smaller than encoder
            dropout=0.1,
            activation='gelu',
            batch_first=True
        )
        self.transformer_decoder = nn.TransformerDecoder(
            ...
            num_layers=2  # Fewer layers than encoder
        )
        ...
        self.signal_decoder = nn.TransformerDecoderLayer(
            d_model=self.hid_dim,
            nhead=4,
            dim_feedforward=512,
            batch_first=True
        )
        self.output_projection = nn.Linear(self.hid_dim, 1)  # Project to single value
    ...
    def patchify_and_mask(self, latent_representation):
        ...
        # patched = latent_representation.clone()  # Save for loss calculation
    ...
    def reconstruct_full_sequence_old(self, patch_info, encoded_visible, visible_mask):
        ...
        mask_tokens = self.mask_token.view(1, -1).expand(len(mask_indices_flat), -1)
    ...
    def reconstruct_full_sequence(self, patch_info, encoded_visible, visible_mask):
        """Reconstruct full sequence including masked tokens using Transformer Decoder."""
        num_samples = patch_info['num_samples']
        num_patches = patch_info['num_patches']
        keep_mask = patch_info['keep_mask']
        
        # 1. Create full sequence with both visible and masked tokens
        full_seq_input = torch.zeros(
            num_samples, num_patches, self.patch_dim, 
            device=self.device
        )
        
        # 2. Prepare visible tokens
        visible_indices = torch.nonzero(visible_mask).squeeze(-1)
        visible_positions = visible_indices % num_patches
        visible_samples = torch.div(visible_indices, num_patches, rounding_mode='floor')
        
        # Place visible encodings in their correct positions
        for i, (sample_idx, pos) in enumerate(zip(visible_samples, visible_positions)):
            full_seq_input[sample_idx, pos] = encoded_visible[i]
        
        # 3. Prepare masked tokens
        mask_indices = torch.nonzero(~keep_mask).squeeze(-1)
        mask_positions = mask_indices % num_patches
        mask_samples = torch.div(mask_indices, num_patches, rounding_mode='floor')
        
        # Place mask tokens in their correct positions
        for i, (sample_idx, pos) in enumerate(zip(mask_samples, mask_positions)):
            # Add positional embedding directly
            full_seq_input[sample_idx, pos] = self.mask_token.view(-1) + self.patch_pos_embed[0, pos]
        
        # 4. Create attention mask to prevent masked tokens from attending to each other
        # (optional, can be removed if you want all tokens to attend to each other)
        attn_mask = torch.zeros(
            num_samples, num_patches, num_patches, 
            device=self.device
        )
        
        for i in range(num_samples):
            # Find which positions are masked for this sample
            sample_masked = ~keep_mask[i]
            # Create mask where masked tokens can't attend to each other
            masked_indices = torch.nonzero(sample_masked).squeeze(-1)
            if len(masked_indices) > 0:
                for idx in masked_indices:
                    attn_mask[i, idx, masked_indices] = float('-inf')
        
        # 5. Reshape for transformer input
        full_seq_input = full_seq_input.reshape(num_samples * num_patches, -1)
        
        # 6. Run transformer decoder
        # Memory = encoded visible tokens
        memory = encoded_visible
        # Target = full sequence with masks
        tgt = full_seq_input 
        
        # Apply transformer decoder
        decoded_sequence = self.transformer_decoder(
            tgt=tgt.unsqueeze(0),  # Add batch dimension
            memory=memory.unsqueeze(0),  # Add batch dimension
            tgt_mask=None  # Remove the problematic mask
        )
        
        # 7. Reshape back to original format
        return decoded_sequence.view(num_samples, num_patches, self.patch_dim)
    ...
    def forward(self, value, truth_time_steps, mask):
        ...
        """
        B, L, D = value.shape
        original_value = value.clone().to(self.device)
        original_mask = mask.clone().to(self.device)
        ...
        output = self.time_att(query_expanded, key_expanded, x_all, mask_all)  # (B*D, 128, hid_dim) --> (B*D, referce_dim, 128) 
        ...
        output = torch.bmm(attention_weights, output).squeeze(1).view(B, D, -1)

        # gru pooling
        ### probably we should map to(B*D, hid_dim)? we can use eather mean polling / attention pooling
        # print('check output shape:', output.shape)

        ##### checking here whether we still need to project to latent space to learn the temporal dependency..        
        
        # ############## PATCHIFYING LATENT REPRESENTATION #############
        # 1. patchify the latent representation
        # patched_output = output.view(B, D, self.patch_size, self.patch_dim)  # (B, D, patch_size, patch_dim)
        ...
        ##3# this seems only work for the fixed length, or we can just pad the originnal singla to the same length
        # reconstructed_signal_flat = self.signal_projector(reconstructed_latent) ## (B*D, max_len)
        # reconstructed_signals = reconstructed_signal_flat.view(B, D, L).transpose(1, 2)  # (B, L, D)
        
        # Create position encoding for each sequence position
        # pos_enc = self.position_encoding(torch.arange(0, L, device=self.device))
        pos_enc = self.position_encoding(torch.arange(0, L, device=self.device).unsqueeze(-1).float())
        
        # Repeat latent for each timestep
        latent_expanded = reconstructed_latent.view(B, D, -1).unsqueeze(1).repeat(1, L, 1, 1)
        
        # Decode signal at each position
        decoded_signals = []
        for i in range(D):
            channel_out = self.signal_decoder(
                pos_enc.unsqueeze(0).repeat(B, 1, 1),  # Query = positions
                latent_expanded[:, :, i, :]  # Key/Value = latent
            )
            decoded_signals.append(self.output_projection(channel_out))
        
        # Combine channels
        reconstructed_signals = torch.cat(decoded_signals, dim=2).to(self.device)  # (B, L, D)
        ...
        # For latent loss calculation:
        if (~keep_mask).sum() > 0:  # If there are masked patches
            masked_patches_orig = orig_patches[~keep_mask.view(B, D, self.patch_size)]
            masked_patches_recon = full_reconstruction[~keep_mask.view(B, D, self.patch_size)]
            latent_loss = F.mse_loss(masked_patches_recon, masked_patches_orig)
        else:
