...
class CLDataCollator:
    ...
    def __call__(self, batch):
        ...
        value_batch = torch.zeros([batch_size, self.max_len, D])
        if self.pretrain_tasks == 'full2':
            mask_batch = torch.zeros([batch_size, self.max_len, 2*D])
        else:
        ...
        for idx in range(batch_size):
            ...
            time_batch[idx, :len1] = t1
            mask_batch[idx, :len1] = m1
        
        return value_batch, time_batch, mask_batch
    ...
    def _per_seq_sampling(self, instance):
        ...
        values, times, mask = instance['data'],instance['time_steps'], instance['mask']
        ...
        # v1 - Contains data points whose indices are in selected_indices
        # These indices are strategically sampled based on temporal density patterns 

        # vw2 - Contains data points whose indices are not in selected_indice
        # Essentially the complement of sequence 1 within the original time series
        ...
        '''
        Before reconstruction mask is applied, the mask_vector is (T, D) tensor
        '''
    ...
    def _time_sensitive_cl(self, timestamps):
        ...
        avg_interval_times = [(((times[i] - times[i - 1]) + (times[i + 1] - times[i])) / 2) for i in range(1, times.shape[0] - 1)]
        ...
        pairs = [(idx, time, avg_interval_time) for idx, (time, avg_interval_time) in enumerate(zip(times, avg_interval_times))]
        # print(pairs)
    ...
    def _seg_masking(self, mask = None, timestamps = None):
        ...
        for dim in range(D):
            ...
            # print(seg_pos)
            '''
            why cannot be the original mask be used as the interpolation mask?
            '''
            if len(seg_pos) > 0:
    ...
    def _constant_length_sampling(self, mask):
        ...
        if self.config.mask_ratio_per_seg * count_ones < 1: seg_seq_len = 1
        else: seg_seq_len = int(self.config.mask_ratio_per_seg * count_ones)
        ...
        seg_pos = []
        for seg in range(self.config.segment_num):
            # print()
            # print(ones_indices_in_mask)

            if len(ones_indices_in_mask) > 1:
                if seg_seq_len == 1: start_idx_in_mask = random.choice(ones_indices_in_mask)
                else: start_idx_in_mask = random.choice(ones_indices_in_mask[ : -seg_seq_len + 1])
    ...
    def _time_sensitive_sampling(self, mask, timestamps):
        ...
        # segment_num = 3 masking_segment J
        # mask_ratio_per_seg = 0.15
        ...
        sampled_times = [timestamps[i].item() for i in range(mask.shape[0]) if mask[i] == 1] # each feature is different then?
        ...
        ### calculating the masking time span??
        time_of_masked_segment = (sampled_times_end - sampled_times_start) * self.config.mask_ratio_per_seg 
        # print('time_of_masked_segment: ' + str(time_of_masked_segment))

        available_samples_to_sample = [time for time in sampled_times if time < sampled_times_end - time_of_masked_segment]
        ...
        masking_times = []
        for i in range(self.config.segment_num):
    ...
    def _seg_sampling(self, max_len):
        if max_len * self.config.mask_ratio_per_seg < 1:
            return []

        seg_pos = []
        seg_len = int(max_len * self.config.mask_ratio_per_seg)
        print('seg_len: ' + str(seg_len))

        start_pos = np.random.randint(max_len, size=self.config.segment_num)
...
# print('test here for understanding')
...
# train_cl_collator = CLDataCollator(max_len = 50, config = config)
# train_cl_collator._time_sensitive_sampling(m, t)
